...
export async function runValidation(
  ...
  for (const name of CHECKS) {
    ...
    let output = "";
    try {
      // Curated env (no owner-shell secrets) for agent-authored scripts;
      // own timer + tree-kill so the whole process tree dies on Windows
      // (execa's timeout kills only the direct cmd.exe child).
      const sp = execa(command, {
        cwd: worktreePath,
        shell: true,
        all: true,
        reject: false,
        windowsHide: true,
        extendEnv: false,
        env: { ...buildScriptEnv(), CI: "true", FORCE_COLOR: "0" },
      });
      let timedOut = false;
      const timer = setTimeout(() => {
        timedOut = true;
        if (sp.pid !== undefined) treeKill(sp.pid, "SIGKILL");
      }, CHECK_TIMEOUT_MS);
      try {
